...
1. **Manifest Loading** (`src/manifest.js`)
   ...
   export const allHooks = {
       'HookTester': [
           () => import('./app/components/HookTester/reactium-hooks-hooktester.js')
       ]
   };
   ```
...
```typescript
class HookSDK {
    register(name: string, callback: Function, order: number, id?: string): string
...
```javascript
import { useRegisterSyncHandle, useSyncHandle } from '@atomic-reactor/reactium-core/sdk';
...
export const Hello = ({ className }) => {
    ...
    return (
        <div className={className}>
            ...
            <nav>
                <ul>
                    <li><Link to='/user/123'>User Page</Link></li>
                    <li><Link to='/data-loader'>Data Loader</Link></li>
                </ul>
            </nav>
        </div>
    );
...
ReactiumWebpack.Hook.registerSync('webpack-config', (sdk) => {
    ...
    sdk.addPlugin('env', new webpack.EnvironmentPlugin({
...
**reactium-hooks.js:**
```javascript
import { Hook, Component, ZoneRegistry } from '@atomic-reactor/reactium-core/sdk';
